...
function hurt(n){let a=Math.min(armor,n*.58);armor-=a;health-=n-a;hitFlash.style.background='#ff231944';setTimeout(()=>hitFlash.style.background='transparent',90);hud();if(health<=0){gameOver=true;banner('SIGNAL LOST // REDEPLOY',3000);setTimeout(respawn,1600)}}
...
function swapWeapon(){currentWeapon=(currentWeapon+1)%weapons.length;ammo=Math.min(ammo,weapons[currentWeapon].mag);hud();banner(`EQUIPPED // ${weapons[currentWeapon].name}`,900)}
function fortify(){let d=new THREE.Vector3();camera.getWorldDirection(d);d.y=0;d.normalize();let cx=Math.round(camera.position.x+d.x*2.5),cz=Math.round(camera.position.z+d.z*2.5),gy=Math.round(highestSurfaceBelow(cx,cz,camera.position.y)+.5),built=0;for(let dx=-2;dx<=2;dx++)for(let yy=0;yy<3;yy++){let x=cx+Math.round(-d.z*dx),z=cz+Math.round(d.x*dx);if(addBlock(x,gy+yy,z,2,true))built++}banner(`FORTIFICATION // +${built} BLOCKS`,900)}
